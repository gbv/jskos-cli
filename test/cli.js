...
const runEnrich = (args) => new Promise(resolve => {
  enrich(args, (error, stdout, stderr) => resolve({ error, stdout, stderr }))
})


describe("jskos-enrich", () => {

  it("should keep enriched subject labels", async () => {
    const expectedFile = file("test/test-subjects-expected.ndjson")
    const expected = fs.readFileSync(expectedFile, "utf8").trim().split("\n")

    const { error, stdout, stderr } = await runEnrich([
      expectedFile,
      "-",
    ])
