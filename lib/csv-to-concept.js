...
module.exports = ({ language, scheme, marktop }) => {
  ...
  return new Transform({
    ...
    transform(row, encoding, callback) {
      ...
      // prefLabel
      const prefLabels = {}
      if (!empty(row.prefLabel)) prefLabels[language] = row.prefLabel
      Object.keys(row)
        .filter(key => /^prefLabel@(.+)$/.test(key))
        .filter(key => !empty(row[key]))
        .forEach(key => { prefLabels[key.substr(10)] = row[key] })

      // scopeNote
      const scopeNotes = {}
      if (!empty(row.scopeNote)) scopeNotes[language] = [ row.scopeNote ]
      Object.keys(row)
        .filter(key => /^scopeNote@(.+)$/.test(key))
        .filter(key => !empty(row[key]))
        .forEach(key => { scopeNotes[key.substr(11)] = [ row[key] ] })
        
      // altLabel
      const altLabels = {}
      if (!empty(row.altLabel)) altLabels[language] = [ row.altLabel ]
      Object.keys(row)
        .filter(key => /^altLabel@(.+)$/.test(key))
        .filter(key => !empty(row[key]))
        .forEach(key => { altLabels[key.substr(9)] = [ row[key] ] })
