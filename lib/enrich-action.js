...
export async function enrichLabels(item, schemes, registries, options = {}) {
  ...
  for (const prop of props) {
    ...
    for (const propertyItem of propertyItems) {
      ...
      const identifier = propertyItem.uri ?? propertyItem.url
      if (!labelsMap.has(identifier)) {
        if (!options.quiet) {
          console.warn(`⚠️  Skipped enrichment for ${identifier}`)
        }
        continue
      }

      const labels = labelsMap.get(identifier)
      if (Object.keys(labels).length > 0) {
        propertyItem.prefLabel = labels
      } else if (!options.quiet) {
        console.warn(`⚠️  No labels returned for ${identifier}`)
      }
    }
  }
