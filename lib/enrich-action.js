...
/**
 * Load config file dynamically based on --scheme option
 * @param {string} schemePath 
 * @returns {Array} configs
 */
async function loadConfigs(schemePath) {
  const filePath = path.resolve(schemePath || "./config/default_config.js")
  const { default: configs } = await import(`file://${filePath}`)
  return configs
}
...
export default async function enrich(files, options = {}) {
  ...
  const configs = await loadConfigs(options.scheme)
