...
function convert (opts) {
  let { from, to, type, language, registry, scheme, destination, marktop, clean, partof, creator } = opts
  ...
  let steps = []
  if (from === "csv") { 
    steps.push(csvParser.parse({ separator: ";", columns: true }))
  } else if(from === "ndjson") {
  ...
  if (type === "mapping") {
    if (from === "ndjson" && to === "csv") {
      ...
      steps.push(mappingsFromRows({ language, fromScheme: scheme, toScheme: destination, partOf: partof, creator }))
    }
    if (opts.validate) {
...
function initializeSerializer(config) {
  ...
  if (to === "csv") {
    ...
    var serializer = csvSerializer.stringify({ delimiter: ";", columns })
